...
df['length'] = df['metadata'].str.extract(".*\,(.*pp.[^\,]*)").fillna('0')                          # Length for text measured in pp
df['length1'] = df['metadata'].str.extract(".*\,(.*ff.[^\,]*)").fillna('0')                         # Helper column for length for text measured in ff
df['length'] = np.where(df['length'] == '0', df['length1'], df['length'])                           # Combining the 2
del df['length1']                                                                                   # Dropping helper col

# Word count column
df['word_count'] = df['metadata'].str.extract("([0-9]*\,*[0-9]*\swords[^\,]*)").fillna('na')

# Time column
df['time'] = df['metadata'].str.extract("([a-z]*\s*[0-9][^\,]*)").fillna('na')
